...
class TextSentimentAnalysisApp:
    ...
    def setResult(self, sentiment_type, result):
        if sentiment_type == "neg":
            self.negativeLabel.configure(text=f"Negative comment detected: {result}%")
        elif sentiment_type == "neu":
            self.neutralLabel.configure(text=f"Neutral comment detected: {result}%")
        elif sentiment_type == "pos":
            self.positiveLabel.configure(text=f"Positive comment detected: {result}%")
    
    def runAnalysis(self):
        sentences = [self.line.get()]
        ...
        for sentence in sentences:
            ...
            for sentiment in sorted(ss):
                self.setResult(sentiment, ss[sentiment])
    ...
    def __init__(self):
        self.main = Tk()
        self.main.title("Text Sentiment Analysis")
